...
// This implementation is for Darwin.
func bindDevice(dialer *net.Dialer, ifaceName string) {
	...
	if foundIP != nil {
		...
		zlog.Infof("%s [Tunnel] Bound to local IP of interface '%s': %s (Darwin)", TAG, ifaceName, foundIP.String())
	} else {
...
// wrapAndroidProtect is a no-op on Darwin.
func wrapAndroidProtect(dialer *net.Dialer) *net.Dialer {
	zlog.Debugf("[Protect] STUB: Compiled for Darwin platform, Socket protection is disabled.")
	return dialer
}
