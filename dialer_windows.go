...
func bindDevice(dialer *net.Dialer, ifaceName string) {
	...
	if foundIP != nil {
		...
		zlog.Infof("%s [Tunnel] Bound to local IP of interface '%s': %s (Windows)", TAG, ifaceName, foundIP.String())
	} else {
...
// wrapAndroidProtect is a no-op on Windows.
func wrapAndroidProtect(dialer *net.Dialer) *net.Dialer {
	zlog.Debugf("[Protect] STUB: Compiled for Windows platform, Socket protection is disabled.")
	return dialer
}
