document.addEventListener('DOMContentLoaded', () => {
    ...
    const modal = {
        ...
        updateVisibility: () => {
            ...
            const isRaw = tunnelType === 'raw';
            const isH2SDK = ['h2', 'grpc', 'h3', 'webtransport', 'masque'].includes(tunnelType);
            const isWebsocket = tunnelType === 'websocket';
            // 合并型隧道（raw/websocket/h2/grpc/xhttp）的 TLS 由开关决定；
            // 其余类型要么固定 TLS（h3/masque/webtransport/quic），要么无 TLS。
            const tlsCapable = isRaw || isWebsocket || tunnelType === 'h2' || isGrpc || isXhttp;
            const tlsOn = tlsCapable && el('tunnelTLSEnabled')?.checked;
            const isWss = isWebsocket; // websocket 家族的代理认证都是 user/pass 形态
            const isCustomPathSupported = ['websocket', 'h2', 'grpc', 'h3', 'webtransport', 'masque', 'xhttp', 'ws', 'wss'].includes(tunnelType);
            ...
            const supportsProxyAuth = ['raw', 'websocket', 'h2', 'grpc', 'h3', 'wt', 'masque', 'xhttp', 'http'].includes(tunnelType);
            ...
            const supportsCertFingerprint = ['quic', 'h3', 'webtransport', 'masque'].includes(tunnelType) || tlsOn;
