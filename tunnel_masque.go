...
func masqueConnect(parentCtx context.Context, cfg ProxyConfig, isH2 bool) (net.Conn, error) {
	protoStr := "HTTP/3"
	if isH2 {
		protoStr = "HTTP/2"
	}
	...
	//  info targetaddress， info  IPv6  info
	...
	case resp := <-respChan:
		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			cancel()
			resp.Body.Close() //  info  HTTP Stream
			rejErr := fmt.Errorf("masque proxy returned status %d", resp.StatusCode)
...
func init() {
	RegisterTunnel("masque", "custom", func(parentCtx context.Context, cfg ProxyConfig, baseConn net.Conn) (net.Conn, error) {
		alpnStr := strings.TrimSpace(strings.ToLower(cfg.Alpn))
		if alpnStr == "h2" {
			return masqueConnect(parentCtx, cfg, true)
		}
		if alpnStr == "h3" {
			return masqueConnect(parentCtx, cfg, false)
		}

		//  info  ALPN  info defaultmode ( info  "h3,h2"、"h2,h3"  info  "")：
		if strings.HasPrefix(alpnStr, "h2") {
			// h2  info ，failed info  h3
			conn, err := masqueConnect(parentCtx, cfg, true)
			if err == nil {
				return conn, nil
			}
			zlog.Warnf("%s [Tunnel] ⚠️ MASQUE H2 failed (%v), falling back to H3...", TAG, err)
			return masqueConnect(parentCtx, cfg, false)
		} else {
			// h3  info （default），failed info  h2
			conn, err := masqueConnect(parentCtx, cfg, false)
			if err == nil {
				return conn, nil
			}
			zlog.Warnf("%s [Tunnel] ⚠️ MASQUE H3 failed (%v), falling back to H2...", TAG, err)
			return masqueConnect(parentCtx, cfg, true)
		}
	})
}
